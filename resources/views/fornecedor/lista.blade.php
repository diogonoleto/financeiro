...
<div class="col-xs-12 no-padding grid-table">
  ...
  @if($i->razao_social != 'Fornecedor Padrão')
  <span class="tools-user">
    @can('fornecedor_update', $i)
    <a href="#" route="{{ route('fornecedor.edit', $i->id) }}" class="btn-empresa-edit"><i class="mdi mdi-pencil mdi-24px"></i></a>
    @endcan
    @can('fornecedor_delete', $i)
    <a href="#" class="btn-tools-delete del-{{ $i->id }}" id="del-{{ $i->id }}" rel="del-{{ $i->id }}" route="{{ route('fornecedor.destroy', $i->id) }}"><i class="mdi mdi-delete mdi-24px"></i></a>
    @endcan
  </span>
  @endif
</div>
...
@empty
<div class="col-xs-12 no-padding grid-table">
  <div class="col-xs-12">Fornecedor inexistente ou não encontrado</div>
</div>
@endforelse
