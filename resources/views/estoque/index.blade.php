...
<section class="content-header">
	...
	<div class="input-group pull-right" id="btn-tools">
		<div class="input-group-btn">
			<a href="#" class="btn btn-default btn-estoque-create" route="{{ route('estoque.create') }}" data-toggle="tooltip" title="Adicionar estoque" data-placement="bottom"><i class="mdi mdi-plus mdi-20px" aria-hidden="true"></i></a>
...
<section class="content">
	<div class="col-sm-12 col-xs-12 no-padding" id="estoque-list">
		<div class="col-xs-12 hidden">
			<form id="form-search" action="{{ route('estoque.lista') }}">
				{!! method_field('GET') !!}
				{!! csrf_field() !!}
		...
		<div class="col-xs-12 no-padding" id="grid-table-header">
			...
			<div class="col-sm-12" style="width: calc(100% - 40px);"><a href="#" class="order" order="nome" sort="ASC">NOME</a></div>
		</div>
		...
		<form id="form-estoque-create" action="" class="hidden" method="GET">
			{{ csrf_field() }}
		</form>
	</div>
	<div class="col-sm-4 col-xs-4 hidden" style="height: 100%; padding: 30px; border-left: 1px solid #8bc34a;" id="estoque-create" >
	</div>
</section>
<!-- /.content -->
@endsection
...
<script type="text/javascript">
  ...
  $(document).on('click', '.btn-estoque-create, .btn-estoque-edit', function(e){
  	e.preventDefault();
  	...
  	if( $(this).hasClass('btn-estoque-create')){
  		$("#estoque-list").toggleClass("col-sm-8").toggleClass("col-xs-8");
  		$("#estoque-create").toggleClass("hidden");
  		$("#form-estoque-create").attr("action", $(this).attr("route")); 
  	} else {
  		$("#estoque-list").addClass("col-sm-8").addClass("col-xs-8");
  		$("#estoque-create").removeClass("hidden");
  		$("#form-estoque-create").attr("action", $(this).attr("route")); 
  	}
  	$("#form-estoque-create").submit();
  });

  $(document).on("submit", "#estoqueForm", function(e) {
  	e.preventDefault();
  	$(".se-pre-con").fadeIn();
  	...
  	$.ajax({
  		...
  		success: function(data){
  			if(data.error){
  				...
  				$("#form-search").submit();
  				$(".btn-estoque-create").trigger("click");
  				$("#estoque-create").html("");
  			}
  			$(".se-pre-con").fadeOut();
  		},
  ...
  $(document).on("submit", "#form-estoque-create", function(e) {
  	e.preventDefault();
  	var url = $(this).attr("action");
  	var get = $(this).attr("method");
  	var data = $(this).serializeArray();
  	$.ajax({
  		...
  		data: data,
  		success: function(data){
  			$("#estoque-create").html(data);
  			
  			$(".se-pre-con").fadeOut();
  			$("#estoque-novo").draggable({
          containment: ".content"
        });
