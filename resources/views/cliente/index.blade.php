...
<section class="content-header">
  ...
  <div class="input-group pull-right" id="btn-tools">
    <div class="input-group-btn">
      <a href="#" class="btn btn-default" id="btn-search" data-toggle="tooltip" title="Pesquisar Cliente" data-placement="bottom"><i class="mdi mdi-magnify mdi-20px"></i></a>
      @can('cliente_create')
      <a href="#" class="btn btn-default btn-empresa-create{{ (isset($qtde_empresa) && $qtde_empresa <= 0) ? ' hidden': null }}" id="btn-empresa-create" qtde="{{ $qtde_empresa }}" route="{{ route('cliente.create') }}" data-toggle="tooltip" title="Adicionar Cliente" data-placement="bottom"><i class="mdi mdi-plus mdi-20px" aria-hidden="true"></i>{{ $qtde_empresa }}</a>
      @endcan
      ...
      @endcan
      @can('cliente_read')
      <a href="{{ route('cliente.index') }}" style="color:#2196f3" class="btn btn-default" data-toggle="tooltip" title="Clientes" data-placement="bottom"><i class="mdi mdi-account-box mdi-20px"></i> {{ $qtde_empresa }}</a>
      @endcan
      @can('fornecedor_read')
...
<section class="content">
  <div class="col-sm-12 col-xs-12 no-padding" id="div-list">
    <div class="col-xs-12 hidden" id="div-search">
      <form id="form-search" action="{{ route('cliente.lista') }}">
        {{ method_field('GET') }}
        {{ csrf_field() }}
