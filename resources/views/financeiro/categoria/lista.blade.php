@foreach($categorias->groupBy('tipo') as $key => $group)
<h4 class="group{{ $key }}">{{ $key }}s</h4>
<ul class="menubar treeview" id="group{{ $key }}">
  @foreach($group as $k => $categoria)
  <li class="menubar-item" rel="{{ $categoria->id }}">
    @if(count($categoria->children))
    <a class="menu-dropdown collapsed" role="button" data-toggle="collapse" href="#cid-{{ $categoria->id }}" aria-expanded="false" aria-controls="{{ $categoria->cod }}" no="{{ $categoria->nome }}" de="{{ $categoria->desc }}">
      <e></e>{{ $categoria->cod }}{{ $categoria->nome }}<div class="desc"> {{ $categoria->desc }}</div>
    </a>
    ...
    @else
    <a href="#" class="submenu-target waves-effect waves-theme" no="{{ $categoria->nome }}" de="{{ $categoria->desc }}">{{ $categoria->cod }}{{ $categoria->nome }}<div class="desc"> {{ $categoria->desc }}</div></a>
