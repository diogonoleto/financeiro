...
<div class="col-xs-12 no-padding grid-table">
  ...
  <span class="tools-user">
    @can('empresa_update', $i)
    <a href="#" route="{{ route('empresa.edit', $i->id) }}" class="btn-empresa-edit"><i class="mdi mdi-pencil mdi-24px"></i></a>
    @endcan
    @can('empresa_delete', $i)
    <a href="#" class="btn-tools-delete del-{{ $i->id }}" id="del-{{ $i->id }}" rel="del-{{ $i->id }}" route="{{ route('empresa.destroy', $i->id) }}"><i class="mdi mdi-delete mdi-24px"></i></a>
    @endcan
  </span>
  @endif
</div>
