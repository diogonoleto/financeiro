...
<section class="content-header">
  ...
  <div class="input-group pull-right" id="btn-tools">
    <div class="input-group-btn">
      <a href="#" class="btn btn-default" id="btn-search" data-toggle="tooltip" title="Pesquisar Empresa" data-placement="bottom"><i class="mdi mdi-magnify mdi-20px"></i></a>
      @can('empresa_create')
      <a href="#" class="btn btn-default btn-empresa-create{{ (isset($qtde_empresa) && $qtde_empresa <= 0) ? ' hidden': null }}" id="btn-empresa-create" qtde="{{ $qtde_empresa }}" route="{{ route('empresa.create') }}" data-toggle="tooltip" title="Adicionar Empresa" data-placement="bottom"><i class="mdi mdi-plus mdi-20px" aria-hidden="true"></i>{{ $qtde_empresa }}</a>
      @endcan
      <div style="width: 5px;display: inline-block;"></div>
      @can('empresa_read')
      <a href="{{ route('empresa.index') }}" style="color:#ffeb3b" class="btn btn-default" data-toggle="tooltip" title="Empresas" data-placement="bottom"><i class="mdi mdi-store mdi-20px"></i></a>
      @endcan
      @can('cliente_read')
...
<section class="content">
  <div class="col-sm-12 col-xs-12 no-padding" id="div-list">
    <div class="col-xs-12 hidden" id="div-search">
      <form id="form-search" action="{{ route('empresa.lista') }}">
        {{ method_field('GET') }}
        {{ csrf_field() }}
